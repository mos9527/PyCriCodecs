/**
 * This whole fils is a copy from VGMStream, all credits to them and its contributors.
 * 
 */
...
int clHCA_isOurFile(const unsigned char *data, unsigned int size);
...
int clHCA_DecodeHeader(clHCA *, unsigned char *data, unsigned int size);
 ...
 * Returns 0 on success, <0 on failure. */
int clHCA_DecodeBlock(clHCA *, unsigned char *data, unsigned int size);
